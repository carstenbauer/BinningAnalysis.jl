...
@testset "All Tests" begin
    ...
    @testset "Jackknife" begin
        ...
        @test jackknife_full(x -> mean(x), ts) == (0.45486176300000025, 0.10834619757501414)
        @test jackknife_full(x -> mean(1 ./ x), ts) == (83.4370988407286, 79.76537738034833)
        @test jackknife_full(g, ts, ts2) == (-0.07916794438503649, 0.14501699232741938)
        ...
        @test jackknife_full(x -> mean(x), ts) == (0.519437840000001 + 0.5584324999999994im, 0.1428607353199917)
        @test jackknife_full(x -> mean(1 ./ x), ts) == (0.6727428408881817 - 0.8112743686359876im, 0.20473472232587642)
        @test jackknife_full(g, ts, ts2) == (0.021446672721215643 + 0.06979705636190503im, 0.2752976586383889)
    end
end
